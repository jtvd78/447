...
unsigned int
div(unsigned int n, unsigned int d)
{
...
unsigned int
mod(unsigned int n, unsigned int d)
{
	while (n > d) {
		n -= d;
	}
...
char *returns[] = {
    ...
    "SD_TIMEOUT",
    "SD_BUSY",
    "SD_WTF_THERE_IS_NO_VALUE_SPECIFIED",
    "SD_NO_RESP",
    "SD_ERROR_RESET",
    "SD_ERROR_CLOCK",
    "SD_ERROR_VOLTAGE",
...
{--------------------------------------------------------------------------*/
#include "time.h"
#define waitMicro wait							// Waits a number of microseconds
#define TICKCOUNT get_time					// Gets current system clock value (1Mhz accuracy)
#define TIMEDIFF(a,b)  (b-a)						// Given two TICKCOUNT values calculates microseconds between them.
...
static SDRESULT sdWaitForData (void) 
{
	...
	if ( (td >= 500000) || (EMMC_INTERRUPT->Raw32 & INT_ERROR_MASK) )
    {
		if (LOG_ERROR) LOG_ERROR("EMMC: Wait for data aborted: %08x %08x %08x\n", 
			...
			uart_puts("EMMC: Wait for data aborted: ");
			uart_put32x(EMMC_STATUS->Raw32);
			uart_puts(" ");
			uart_put32x(EMMC_INTERRUPT->Raw32);
			uart_puts(" ");
			uart_put32x(*EMMC_RESP0);
			uart_puts("\n");
		}
		return SD_BUSY;												// return SD_BUSY
    }
...
//	log("EMMC: Sending command code ", cmd->code.CMD_INDEX);
	log((char *)cmd->cmd_name, arg);
...
{
//	log("sdSendCmdA idx ", index);
//	log("sdSendCmdA arg ", arg);
	// Issue APP_CMD if needed.
	SDRESULT resp;
	if( index >= IX_APP_CMD_START && (resp = sdSendAppCommand()) )
		return sdDebugResponse(resp);
...
{
	uint32_t divisor = div(41666667 + freq - 1, freq);				// Pi SD frequency is always 41.66667Mhz on baremetal
...
static SDRESULT sdSetClock (uint32_t freq)
{
	...
	log("EMMC: setting clock speed to ", freq);
...
uart_puts("sdTransferBlocks ");
if (write) uart_puts("write blk "); else uart_puts("read blk "); 
uart_put32x((long)startBlock);
uart_puts(" len ");
uart_put32x((long)numBlocks);
uart_puts(" addr ");
uart_put32x((long)buffer);
uart_puts("\n");

//uart_puts("TB lr "); uart_put32x(lr); uart_puts(" sp "); uart_put32x(sp); uart_puts("\n");

	if ( sdCard.type == SD_TYPE_UNKNOWN ) return SD_NO_RESP;		// If card not known return error
	if ( sdWaitForData() ) {
		uart_puts("sdWaitForData() returned non-zero\n");
		return SD_TIMEOUT;						// Ensure any data operation has completed before doing the transfer.
	}
...
static bool LoadDrivePartition (printhandler prn_basic) {
	...
	partition_totalClusters = div(sdCard.partition.dataSectors, sdCard.partition.sectorPerCluster);
...
uint32_t getSetNextCluster (uint32_t clusterNumber, bool set, uint32_t clusterEntry) {
	...
	FATEntrySector = sdCard.partition.unusedSectors + sdCard.partition.reservedSectorCount +
		div((clusterNumber * 4), sdCard.partition.bytesPerSector);	// Get sector number of the cluster entry in the FAT
	FATEntryOffset = mod((clusterNumber * 4), sdCard.partition.bytesPerSector); // Get the offset address in that sector number
...
char *getbuf(int num)
{
	return (char *)&(intSearchRecord[num].buffer[0]);
}
	...
	if (LFdir) {													// Check the directory pointer valid
		for (j = 0; ((j < 5) && (lfn_done == false)); j++) {		// For each of the first 5 characters
			...
			if (utf == 0) lfn_done = true;							// If the character is zero we are done
				else LFNtext[LFN_blockcount++] = (char)(utf&0xFF);	// Otherwise narrow to ascii and place in buffer
				//else { uart_puts("ERROR: Fucking thing uses UTF\n"); }
		}
...
static struct dir_Structure* LocateFATEntry (const char* searchPat, struct PRIV_SEARCH_DATA* priv, bool dirChange, LFN_NAME LFN_Name, uint32_t* ErrorID) {
	...
	uart_puts("LocateFATEntry: ["); uart_putns((char *)searchPat, 10); uart_puts("]\n");
...
//				uart_puts(" name: ["); uart_putns(dir->name, 16); uart_puts("]\n");
...
//	while ((intSearchRecord[i].firstSector != 0) && i < MAX_SEARCH_RECORDS) i++;
	...
	uart_puts("sdFindFirstFile - intSearchRecord[].firstSector = ");
	uart_put32x(intSearchRecord[i].firstSector);
	uart_puts("\n");

		if ((ret = sdTransferBlocks(intSearchRecord[i].firstSector, 1,
			&intSearchRecord[i].buffer[0], false)) == SD_OK) {
			char* p;
			do {
				p = strchr((char *)searchStr, '\\');						// Check for sub-directory slash
		...
		uart_puts(" 1 dir = "); uart_put32x((long)dir); uart_puts("\n");
		uart_puts(" 1 errID = "); uart_putns(faterrs[errID], 24); uart_puts("\n");
		...
		uart_puts(" 2 dir = "); uart_put32x((long)dir); uart_puts("\n");
		uart_puts(" 2 errID = "); uart_putns(faterrs[errID], 24); uart_puts("\n");
			...
			uart_puts("sdTransferBlocks failed, ret [");
			uart_putns(returns[ret], 24);
			uart_puts("]\n");
		} else {
			uart_puts("sdTransferBlocks failed, ret out of bounds = ");
			uart_put32x(ret);
			uart_puts("\n");
		}
		intSearchRecord[i].firstSector = 0;							// Clear first sector which is used to mark in use
	} else {
		uart_puts("i == MAX_SEARCH_RECORDS\n");
	}
...
HANDLE sdFindNextFile (HANDLE hFindFile, FIND_DATA* lpFFData)
{
	uint32_t errID = FAT_RESULT_OK;
	struct dir_Structure* dir;
	uart_puts("sdFindNextFile - intSearchRecord[].firstSector = ");
	uart_put32x(intSearchRecord[hFindFile-1].firstSector);
	uart_puts("\n");
	if ((hFindFile > 0) && (hFindFile <= MAX_SEARCH_RECORDS) &&
		...
		intSearchRecord[hFindFile-1].searchPattern[0] = '*';
		intSearchRecord[hFindFile-1].searchPattern[1] = '.';
		intSearchRecord[hFindFile-1].searchPattern[2] = '*';
		intSearchRecord[hFindFile-1].searchPattern[3] = '\0';
		dir = LocateFATEntry(&intSearchRecord[hFindFile-1].searchPattern[0],
			&intSearchRecord[hFindFile-1],
			false, lpFFData->cFileName, &errID);					// Locate first FAT entry
		uart_puts(" dir = "); uart_put32x((long)dir); uart_puts("\n");
		uart_puts(" errID = "); uart_putns(faterrs[errID], 24); uart_puts("\n");
		if ((errID == FAT_RESULT_OK) && (dir)) {					// First FAT entry returned
			SetFindDataFromFATEntry(dir, lpFFData);					// Set all the find data fields from FAT entry
			return (hFindFile);										// Return internal search record index as handle
		}
...
//uart_put32x(__LINE__); uart_puts(" i="); uart_put32x(i); uart_puts("\n");
i=0;
//uart_put32x(__LINE__); uart_puts(" i="); uart_put32x(i); uart_puts("\n");
...
//uart_put32x(__LINE__); uart_puts(" i="); uart_put32x(i); uart_puts("\n");
				...
				{
//uart_put32x(__LINE__); uart_puts(" i="); uart_put32x(i); uart_puts("\n");
//uart_puts("CF lr "); uart_put32x(lr); uart_puts(" sp "); uart_put32x(sp); uart_puts("\n");
//uart_putns((char *)intFileIORecord[i].srec.buffer, 64); uart_puts("\n");
//uart_puts("sdTransfer success = "); uart_put32x(i+1); uart_puts("; return from sdCreateFile\n");
//uart_puts("CF lr "); uart_put32x(lr); uart_puts(" sp "); uart_put32x(sp); uart_puts("\n");
					return (i + 1);									// Return internal file io record index as handle
				} else uart_puts("sdTransferBlocks for dir fail\n");
			} else uart_puts("LocateFatEntry not OK\n");
		} else uart_puts("sdTransferBlocks not SD_OK\n");
	}
...
SDRESULT sdInitCard (printhandler prn_basic, printhandler prn_error, bool mount)
{
	...
	if (sdCard.scr.BUS_WIDTH == BUS_WIDTH_4)
	{
		...
		log("EMMC: Bus width set to 4", NOVAL);
	}
...
char *getbuf2(int num)
{
	return (char *)&intFileIORecord[num].srec.buffer[0];
}

int getFileSize(int idx)
{
	return intFileIORecord[idx-1].fileSize;
}
