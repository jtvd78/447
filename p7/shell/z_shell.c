...
//	int j=10;
	...
	puts("\nRunning the eggshell on core "); put4x(syscall_get_cpuid()); puts(".\n");
...
*/
		...
		switch(cmd[0].num) {
			case CMD_PS:
				...
				break;
			case CMD_RUN:
				puts("CMD_RUN ["); puts(cmd[1].name); puts(", "); puts(cmd[2].ptr); puts("]\n");
				syscall_start_thread(cmd[1].num, (char *)cmd[2].ptr);
				break;
			case CMD_DUMP:
