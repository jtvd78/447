...
do_dev_word(struct event *ep)
{
	...
	if ((iop->rw == IO_READ && devtab[iop->device].rcheck()) ||
		(iop->rw == IO_WRITE && devtab[iop->device].wcheck())) {
		ep->max_repeats = 0;
		ep->repeat_interval = 0;
		if (iop->rw == IO_WRITE) {
			ch = devtab[iop->device].write(iop->data.num);
		} else {
			ch = devtab[iop->device].read();
		}
		wake_thread(iop->threadid, ch);
		free_io_entry(iop);
	} else if (ep->max_repeats == 0) {
		// we failed at our last chance ... return fail
		ep->max_repeats = 0;
		ep->repeat_interval = 0;
		wake_thread(iop->threadid, DEAD);
		free_io_entry(iop);
	} else {
		ep->max_repeats = 2;
		ep->repeat_interval = MSEC(50);
		ep->timeout = MSEC(50);
		return;
		// exponential back-off?
		ep->repeat_interval *= 2;
	}
	return;
}
